...
| Click a match leaf | Opens that file in a permanent tab with the match selected and scrolled into view — identical to today's behavior |
| Click a file branch row | Opens that file in a permanent tab, with no selection change in the editor beyond whatever the file's own cursor position is |
| Click a folder branch row | Only expands/collapses it; nothing opens |
| Arrow-key down through the results tree (after Tab/click into it) | Each match or file row opens as it's reached, mirroring the file tree's own arrow-key-previews-a-file behavior |
