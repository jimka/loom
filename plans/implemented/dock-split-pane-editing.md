...
**Step 1's `@jimka/typescript-ui` version bump did not happen, deliberately.** `node_modules/@jimka/typescript-ui` is a symlink straight into the sibling `typescript-ui` repo's `code-editor-selection-api` worktree — the branch that carries the five `Dock` methods and two events this plan depends on — bypassing normal package resolution entirely. There is no published version that carries them; the library work lives on an unmerged, unreleased branch. `package.json`'s `"@jimka/typescript-ui": "^0.8.0"` was left untouched. This is a known, deliberate integration state for this pipeline, not a deviation caused by this plan's implementation.

**`findByPath` absorbed a pre-existing, functionally-identical private method.** By the time this plan reached the codebase, an earlier phase (`project-wide-replace`) had already added a private `findOpenFile(path)` to `EditorController`, used by `performReplaceInFile` and `replaceMatch` — a method the plan's own drafting could not have known about, since it predates that phase landing on this stacked branch. Rather than adding the plan's `findByPath` as a second, near-identical linear scan alongside the existing `findOpenFile`, the implementation consolidated them: `findOpenFile` was deleted and both of its call sites route through `findByPath` instead. This is a deviation from the plan's literal text (which only names `openFile`, `openFileAt`, `markExternalChanges`, and `saveAs`'s duplicate-target guard as `findByPath`'s call sites), made to avoid shipping two copies of the same lookup after drift introduced the first one.

**A pre-existing, unrelated `@jimka/typescript-ui` bug does manifest in this plan's restore path, and is left unfixed per `CLAUDE.md`.** The bug: `Dock.setLayoutState` delegates to `LayoutSerialization.ts`'s `restoreLayout`/`populateContainer`, which rebuilds the *root* region in place — `container.setLayoutManager(new Tab(...))` on the same live `Component` `Dock.getRootRegion()` always returns — rather than replacing the container itself. `wireRegion`'s idempotency ledger (`_wiring`, keyed by `Component` identity) has no way to see that in-place manager swap: once the root has been wired once (which happens as soon as the dock's constructor-scheduled initial sweep runs, well before `restoreFiles` finishes reading every restored file's text off disk and calls `setLayoutState`), `wireRegion`'s `!wiring.tabWired` guard is permanently false for that `Component`, so the fresh `Tab` `populateContainer` just installed never receives `applyTabOptions` (the dock's `widthMode`/`maxWidth`/`scrollable` settings) or any of `wireRegion`'s event wiring (the listeners that let `beforeclose`/`close`/`focus`/`dblclick` reach `EditorController`'s handlers) — and the flag never resets, so every *later* panel added into that same root region for the rest of the app's life is affected too, not just the ones present at restore.

This is not a rare edge case: `Dock` always wraps its root in a `Tab` (even for one open file), and `captureEditorLayout()` only returns `null` when the dock is entirely empty, so any restore with at least one remembered file open drives `setLayoutState` down this path. The concrete, confirmed (via direct reading of `Dock.ts` and `LayoutSerialization.ts` in the sibling `typescript-ui` worktree) symptom for Loom: after a restart that reopens any file into the dock's original (root) group, that group's tab strip stops honouring `tabMaxWidthPx`/content-width/scrollable, and — more seriously — closing a dirty tab there no longer raises the unsaved-changes prompt, because the `beforeclose` veto path this plan's `handleBeforePanelClose` depends on never got wired onto that region's `Tab` in the first place. A *second* group, created by dragging a tab out of the root region after restore, is unaffected — the split machinery gives it a genuinely new `Component`, wired normally.

Per `CLAUDE.md`'s rule against papering over a library gap with a Loom-specific workaround, this is reported rather than fixed here: it is a defect in `Dock.setLayoutState`/`restoreLayout`'s interaction with `wireRegion`'s idempotency guard, not in anything this plan's code owns, and fixing it from the Loom side would mean reproducing `wireRegion`'s private sweep logic — exactly the workaround `## Upstream Prerequisites`'s own rejected-workaround footnote already argues against. It was not caught by this plan's automated tests because `readLayoutState`/`remapPanelIds`/`panelOrder` are pure data-shape functions with no live `Dock` in the loop, and it needs a real `Dock` instance and a real restore sequence to reproduce — exactly the class of behaviour this plan's own `## Expected Behaviour` marks manual-verify-only.

**The manual-verification table was not run.** This environment has no live Tauri window — `npm run tauri:dev` cannot be driven here, and the sandbox's own pre-existing `src/data/workspace.ts` Tauri-plugin crash blocks even the chrome-devtools-driven fallback other Loom work in this environment has used. Every row of `## Expected Behaviour`'s manual-verification table (drag-to-split, cross-group drag, tear-off floats, the restart round-trip, and the rest) is therefore unverified beyond code review and the automated suite — including, notably, the `setLayoutState` wiring bug just described, whose most visible symptom (a restored group's missing unsaved-changes prompt) is exactly the kind of thing that table would have caught live. This is the test-first escape hatch's honest substitute, not a silent skip: the expected behaviour was described first (in the plan), the code was implemented to match it, and what remains is the manual walk-through the plan itself calls for, on a machine with a working Tauri window.
