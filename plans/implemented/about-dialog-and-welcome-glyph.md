...
- **`buildMenuBar`'s trailing comment avoids the literal substring `setMenus`.** The `## Internal Structure` snippet's comment reads "...never calls setMenus again..." — but the plan's own `## Verification` list requires `grep -n 'setMenus' src/shell/EditorShell.ts` to return zero matches, and a plain grep does not distinguish prose from a call site. Implemented the comment as "...never resets the bar's children afterwards..." instead, preserving the same explanation with no `setMenus` call anywhere in the file (there never was one) and no substring match either.
- **`grep -rn 'APP_TAGLINE' src/` returns three matches, not the two `## Verification` names.** `src/shell/aboutDialog.ts` needs both an `import { ..., APP_TAGLINE } from '../appIdentity'` line and a `${APP_TAGLINE}` usage line inside the template literal — two grep-countable lines in that one file, plus the definition in `src/appIdentity.ts`, for three total. The verification text's "the use in `src/shell/aboutDialog.ts`" reads as one location, not one grep line; there is no way to reference `APP_TAGLINE` from another module without an import statement, so three matches is the correct, unavoidable count for the code as specified.
